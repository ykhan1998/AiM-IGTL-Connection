...
classdef kWaveDiffusion < handle
    ...
    methods
        function kdiff = kWaveDiffusion(kgrid, medium, source, cem_temp, sensor, varargin)
            
            % -------------------------------------------------------------
            kdiff.cem_temp = cem_temp;
            ...
            if (nargin >= 5) && ~isempty(sensor) && isfield(sensor, 'mask')
...
%             end
            ...
            for t_index = 1:Nt
                ...
                obj.cem43 = obj.cem43 + dt ./ 60 .* ( 0.5 .* (obj.T >= obj.cem_temp) + 0.25 .* (obj.T >= 20 & obj.T < obj.cem_temp ) ).^(obj.cem_temp - obj.T);
                % obj.cem43 = obj.cem43 + dt ./ 60 .* ( 0.5 .* (obj.T >= 25) + 0.25 .* (obj.T >= 18 & obj.T < 25 ) ).^(25 - obj.T);
...
%                     caxis([20,50]);
    ...
    methods
        ...
        function lesion_map = get.lesion_map(obj)
            ...
            % KT changed this to reflect cem43 thresholds for brain
            %lesion_map = obj.cem43 >= 1;
            lesion_map = obj.cem43 >= 10;
        end
